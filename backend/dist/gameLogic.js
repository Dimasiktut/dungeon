...
export function handleStartGame(roomId, playerId) {
    ...
    if (room.hostId !== playerId) {
        ...
        if (player)
            player.ws.send(JSON.stringify({ type: ServerMessageType.ERROR, payload: { message: "Only the host can start the game." } }));
        return;
    }
    if (room.players.length < MIN_PLAYERS_TO_START) {
        const player = room.players.find(p => p.id === playerId);
        if (player)
            player.ws.send(JSON.stringify({ type: ServerMessageType.ERROR, payload: { message: `Need at least ${MIN_PLAYERS_TO_START} players.` } }));
        return;
    }
    if (room.gameState && room.gameState.isGameStarted) {
        const player = room.players.find(p => p.id === playerId);
        if (player)
            player.ws.send(JSON.stringify({ type: ServerMessageType.ERROR, payload: { message: "Game already in progress." } }));
        return;
    }
...
export function handleKickOpenDoor(roomId, playerId) {
    const room = getRoom(roomId);
    ...
    if (gs.currentPlayerId !== playerId || gs.turnPhase !== TurnPhase.KickOpenDoor) {
        const player = room.players.find(p => p.id === playerId);
        if (player)
            player.ws.send(JSON.stringify({ type: ServerMessageType.ERROR, payload: { message: "Not your turn or wrong game phase." } }));
        return;
    }
...
export function handleResolveDoorCard(roomId, playerId, action) {
    const room = getRoom(roomId);
    if (!room || !room.gameState || !room.gameState.isGameStarted)
        return;
    const gs = room.gameState;
    ...
    if (!player || gs.currentPlayerId !== playerId || gs.turnPhase !== TurnPhase.ResolveDoorCard || !gs.currentEncounterCard) {
        ...
        if (requestingPlayer)
            requestingPlayer.ws.send(JSON.stringify({ type: ServerMessageType.ERROR, payload: { message: "Cannot resolve card now." } }));
        return;
    }
...
export function handleLootRoom(roomId, playerId) {
    const room = getRoom(roomId);
    if (!room || !room.gameState || !room.gameState.isGameStarted)
        return;
    const gs = room.gameState;
    ...
    if (!player || gs.currentPlayerId !== playerId || gs.turnPhase !== TurnPhase.LootTheRoom) {
        const requestingPlayer = room.players.find(p => p.id === playerId);
        if (requestingPlayer)
            requestingPlayer.ws.send(JSON.stringify({ type: ServerMessageType.ERROR, payload: { message: "Cannot loot now." } }));
        return;
    }
...
export function handlePlayCardFromHand(roomId, playerId, cardId) {
    const room = getRoom(roomId);
    if (!room || !room.gameState || !room.gameState.isGameStarted)
        return;
    const gs = room.gameState;
    ...
    if (!player || gs.currentPlayerId !== playerId) {
        const requestingPlayer = room.players.find(p => p.id === playerId);
        if (requestingPlayer)
            requestingPlayer.ws.send(JSON.stringify({ type: ServerMessageType.ERROR, payload: { message: "Cannot play card now (not your turn)." } }));
        return;
    }
    ...
    if (![TurnPhase.LookingForTrouble, TurnPhase.KickOpenDoor, TurnPhase.ResolveDoorCard, TurnPhase.Combat, TurnPhase.Charity, TurnPhase.LootTheRoom].includes(gs.turnPhase)) {
        const requestingPlayer = room.players.find(p => p.id === playerId);
        if (requestingPlayer)
            requestingPlayer.ws.send(JSON.stringify({ type: ServerMessageType.ERROR, payload: { message: "Cannot play cards at this time (invalid phase)." } }));
        return;
    }
...
export function handleEndTurn(roomId, playerId) {
    const room = getRoom(roomId);
    if (!room || !room.gameState || !room.gameState.isGameStarted)
        return;
    const gs = room.gameState;
    ...
    if (!player || gs.currentPlayerId !== playerId) {
        const requestingPlayer = room.players.find(p => p.id === playerId);
        if (requestingPlayer)
            requestingPlayer.ws.send(JSON.stringify({ type: ServerMessageType.ERROR, payload: { message: "Cannot end turn now." } }));
        return;
    }
    // Charity Phase

...
import {
    ActiveRoom, Player, RoomSettings, ServerMessageType,
...
export function findRoomByPlayerId(playerId: string): ActiveRoom | undefined {
...
export function broadcastToRoom(roomId: string, message: object, excludePlayerId?: string) {
...
export function handleCreateRoom(ws: WebSocket, payload: CreateRoomPayload) {
    const roomId = uuidv4().slice(0, 6).toUpperCase();
    const hostId = (ws as any).clientId;
...
export function handleJoinRoom(ws: WebSocket, payload: JoinRoomPayload) {
    ...
    const playerId = (ws as any).clientId;
        ...
        if (playerIndex > -1) {
            ...
            if (room.gameState && room.gameState.isGameStarted) {
                ...
                if (gs.players.length === 0) {
                    ...
                    let newPlayerForTurn: Player | undefined = undefined;
...
function addLogEntry(gameState: GameState | null, message: string) {
    if (!gameState) return;
...
export function addChatMessageToRoom(roomId: string, player: Player, text: string) {
    const room = activeRooms.get(roomId);
    if (!room) return;
